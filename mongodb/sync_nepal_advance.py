...
from config import (
    MONGODB_URI,
    DATABASE_NAME,
    COLLECTION_NEPAL_ADVANCE,
    IST
)


class NepalAdvanceMongoDBSync:
    def __init__(self):
        if not MONGODB_URI:
    ...
    def sync_advance_detailed(self, filepath):
        """Sync Nepal advance booking data as single date-specific document"""
        data = self.load_json(filepath)
        if not data:
            print("⚠️  No valid advance data to sync")
            return False

        shows = data.get("shows", [])
        if not shows:
            print("⚠️  No shows in advance data")
            return False
        ...
        collection = self.db[COLLECTION_NEPAL_ADVANCE]

        print(f"\n📊 Syncing Nepal advance booking data for {date_code}...")
        ...
        try:
            ...
            if result.upserted_id:
                print(f"   ✅ Inserted: advance_{date_code}")
            else:
                print(f"   ✅ Updated: advance_{date_code}")
    ...
    def create_indexes(self):
        ...
        try:
            ...
            print(f"✅ Indexes created for {COLLECTION_NEPAL_ADVANCE}")
        except Exception as e:
            print(f"⚠️  Index creation warning: {e}")

    def sync_all(self):
        """Sync advance detailed data"""
        detailed_file = self.find_latest_detailed_file()

        if not detailed_file:
            print("\n❌ No advance file found to sync")
            return False

        # Sync advance data
        success = self.sync_advance_detailed(detailed_file)
...
def main():
    print("🚀 Starting Nepal Advance Booking MongoDB Sync...")
    print(f"⏰ Time: {datetime.now(IST).strftime('%Y-%m-%d %H:%M:%S IST')}")

    syncer = NepalAdvanceMongoDBSync()
    ...
    try:
        ...
        if success:
            print("\n" + "="*60)
            print("✅ NEPAL ADVANCE BOOKING SYNC COMPLETED")
            print("="*60)
            sys.exit(0)
        else:
            print("\n" + "="*60)
